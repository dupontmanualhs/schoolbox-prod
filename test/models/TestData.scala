...
object TestData {
  ...
  def loadScheduleData(debug: Boolean = false)(implicit pm: ScalaPersistenceManager) {
    ...
    //Questions
    //Questions are individual questions
    //Questions are declared as new Question("question text", List("ans1", "ans2", "etc."), value (optional))
    if(debug) println("Creating Questions")
    val primeFactor1 = new Question("50",List("2*5*5","2*5^2"))
    val primeFactor2 = new Question("180", List("2*2*3*3*5", "2^2*3^2*5"), 2)
    val simpRad1A = new Question("sqrt(50)", List("5sqrt(2)"))
    val simpRad1B = new Question("sqrt(180)",List("6sqrt(5)"))
    val simpRad1C = new Question("sqrt(10) * sqrt(30)",List("10sqrt(3)"), 3)
    val simpRad2A = new Question("sqrt(2/25)",List("sqrt(2)/5", "1/5sqrt(2)"))
    val simpRad2B = new Question("5sqrt(7) * sqrt(7)",List("35"))
    val simpRad2C = new Question("sqrt(3) + sqrt(3)",List("2sqrt(3)"))
    val sentence1A = new Question("You can only add monomials if they have the same____",List("variable with the same exponents"))
    val sentence1B = new Question("When you are ading monomials, _____ coefficients",List("add"))
    val sentence2A = new Question("When you are adding monomials the bases and exponents _____",List("stay the same"))
    val sentence2B = new Question("Allen is _____",List("amazing", "awesome", "a great person", "A wonderful friend"),100)
    val simpExprss1A = new Question("-4a + a",List("-3a"))
    val simpExprss2A = new Question("(a^2)^4", List("a^8"))
    val simpExprss3 = new Question("a * a",List("a^2"))
    val simpExprss2B = new Question("7a * 3a", List("21a^2"))
    val simpExprss1B = new Question("(a + b)^2", List("a^2+2ab+b^2", "a^2+b^2+2ab", "b^2+a^2+2ab", "b^2+2ab+a^2"))
    if(debug) println("Adding Questions")
    pm.makePersistentAll(List(primeFactor1, primeFactor2, simpRad1A, simpRad1B, simpRad1C,
        simpRad2A, simpRad2B, simpRad2C, sentence1A, sentence1B, sentence2A, sentence2B,
        simpExprss1A, simpExprss2A, simpExprss3, simpExprss1B, simpExprss2B))
    //Question Sets
    ...
    val simpRadQSet1 = new QuestionSet(List(simpRad1A, simpRad1B, simpRad1C))
    val simpRadQSet2 = new QuestionSet(List(simpRad2A, simpRad2B, simpRad2C))
    val SentenceQSet1 = new QuestionSet(List(sentence1A, sentence1B))
    val SentenceQSet2 = new QuestionSet(List(sentence2A, sentence2B))
    val simpExprssQSet1 = new QuestionSet(List(simpExprss1A, simpExprss1B))
    val simpExprssQSet2 = new QuestionSet(List(simpExprss2A, simpExprss2B))
