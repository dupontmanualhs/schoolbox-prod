...
object TestData {
  ...
  def loadScheduleData(debug: Boolean = false)(implicit pm: ScalaPersistenceManager) {
    ...
    val primeFactor1 = new Question("50",List("2*5*5","2*5^2"),1)
    val primeFactor2 = new Question("180", List("2*2*3*3*5", "2^2*3^2*5"), 1)
    val simpRad1 = new Question("sqrt(50)", List("5sqrt(2)"), 1)
    val simpRad2 = new Question("sqrt(180)",List("6sqrt(5)"),1)
    val simpRad3 = new Question("sqrt(10) * sqrt(30)",List("10sqrt(3)"),1)
    val simpRad4 = new Question("sqrt(2/25)",List("sqrt(2)/5", "1/5sqrt(2)"),1)
    val simpRad5 = new Question("5sqrt(7) * sqrt(7)",List("35"),1)
    val simpRad6 = new Question("sqrt(3) + sqrt(3)",List("2sqrt(3)"),1)
    val sentence1 = new Question("You can only add monomials if they have the same____",List("variable with the same exponents"),1)
    val sentence2 = new Question("When you are ading monomials, _____ coefficients",List("add"),1)
    val sentence3 = new Question("When you are adding monomials the bases and exponents _____",List("stay the same"),1)
    val sentence4 = new Question("Allen is _____",List("amazing", "awesome", "a great person", "A wonderful friend"),1)
    val simpExprss1 = new Question("-4a + a",List("-3a"),1)
    val simpExprss2 = new Question("(a^2)^4", List("a^8"), 1)
    val simpExprss3 = new Question("a * a",List("a^2"),1)
    val simpExprss4 = new Question("7a * 3a", List("21a^2"), 1)
    val simpExprss5 = new Question("(a + b)^2", List("a^2+2ab+b^2", "a^2+b^2+2ab", "b^2+a^2+2ab", "b^2+2ab+a^2"), 1)
