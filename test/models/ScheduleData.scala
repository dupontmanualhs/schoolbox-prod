...
object ScheduleData {
  ...
  def load(debug: Boolean = false)(implicit pm: ScalaPersistenceManager) {
    ...
    val maryAssignment = List(r1planning, r2usHistory, w1worldHistory, w2studySkill)
    for(cls <- maryAssignment){
      val enr = new TeacherAssignment(maryTeacher, cls, fall2012, null, null)
      val enr2 = new TeacherAssignment(maryTeacher, cls, spring2013, null, null)
      pm.makePersistentAll(List(enr, enr2))
    }
    
    val tinaAssignment = List(r1english1, r2planning, w1english2, w2english3)
    for(cls <- tinaAssignment){
      val enr = new TeacherAssignment(christinaTeacher, cls, fall2012, null, null)
      val enr2 = new TeacherAssignment(christinaTeacher, cls, spring2013, null, null)
      pm.makePersistentAll(List(enr, enr2))
    }
    
    val toddAssignment = List(r1algebra1, r2algebra2, w1planning, w2geometry)
    for(cls <- toddAssignment){
      val enr = new TeacherAssignment(toddTeacher, cls, fall2012, null, null)
      val enr2 = new TeacherAssignment(toddTeacher, cls, spring2013, null, null)
      pm.makePersistentAll(List(enr, enr2))
    }
    
    val richAssignment = List(r1planning, r2usHistory, w1worldHistory, w2studySkill)
    for(cls <- richAssignment){
      val enr = new TeacherAssignment(richardTeacher, cls, fall2012, null, null)
      val enr2 = new TeacherAssignment(richardTeacher, cls, spring2013, null, null)
      pm.makePersistentAll(List(enr, enr2))
    }
