...
object TeacherSchedule {
  def apply(teacher: Teacher, term: Term, 
      rows: Seq[STag], hasAssignments: Boolean)(implicit req: VisitRequest[_], config: Config) = {
...
object MasterList {
  def apply(sections: List[Section])(implicit req: VisitRequest[_], config: Config) = {
    ...
    config.main(title, script.src(config.webjars("jquery.dataTables.js")))(
      div.cls("page-header")(
        h2(title)),
